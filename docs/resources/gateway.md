...
* `vpc_id` - (Required) VPC/VNet ID where the Gateway will be deployed.  For HUB mode deployments, the value must refer to the **id** attribute of the [`ciscomcd_service_vpc`](/terraform/ciscomcd_service_vpc/#ciscomcd_service_vpc) resource.
...
* `log_profile` - (Optional) Log Profile ID *(e.g. ciscomcd_profile_log_forwarding.splunk1.id)*
* `packet_capture_profile` - (Optional) Packet Profile ID *(e.g. ciscomcd_profile_packet_capture.pcap1.id)*
* `diagnostics_profile` - (Optional) Diagnostics Profile ID *(e.g. ciscomcd_profile_diagnostics.diag1.id)*
